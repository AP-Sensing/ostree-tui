...
class SnappyWindowImpl : public ComponentBase, public WindowOptions {
 public:
  explicit SnappyWindowImpl(WindowOptions option) : WindowOptions(std::move(option)) {
    if (!inner) {
...
Component SnappyWindow(WindowOptions option) {
  return Make<SnappyWindowImpl>(std::move(option));
}
