...
auto renderDefault(cpplibostree::OSTreeRepo repo,
	...
	for (auto visible_commit_index : visible_commit_map) {
		...
		if (! used_branches.at(relevant_branch)) {
			for (auto branch : used_branches) {
				if (branch.second) {
					tree_branch_elements.push_back(text(COMMIT_TREE) | color(branch_color_map[branch.first]));
				} else {
					tree_branch_elements.push_back(text(COMMIT_NONE) | color(branch_color_map[branch.first]));
				}
			}
		...
		for (auto branch : used_branches) {
			if (branch.second) {
				if (branch.first == relevant_branch) {
					tree_top_elements.push_back(text(COMMIT_NODE) | color(branch_color_map[branch.first]));
				} else {
					tree_top_elements.push_back(text(COMMIT_TREE) | color(branch_color_map[branch.first]));
				}
				tree_root_elements.push_back(text(COMMIT_TREE) | color(branch_color_map[branch.first]));
				tree_bottom_elements.push_back(text(COMMIT_TREE) | color(branch_color_map[branch.first]));
				sign_elements.push_back(text(COMMIT_TREE) | color(branch_color_map[branch.first]));
			} else {
				tree_top_elements.push_back(text(COMMIT_NONE) | color(branch_color_map[branch.first]));
				tree_root_elements.push_back(text(COMMIT_NONE) | color(branch_color_map[branch.first]));
				tree_bottom_elements.push_back(text(COMMIT_NONE) | color(branch_color_map[branch.first]));
				sign_elements.push_back(text(COMMIT_NONE) | color(branch_color_map[branch.first]));
			}
			++branch_index;
		}
		...
		if (commit_top_text.size() > 8)
			commit_top_text = GAP + commit.hash.substr(commit.hash.size() - 8);
		...
		comm_elements.push_back(text(GAP + commit.date));
		// signed
		if (repo.isCommitSigned(commit)) {
			tree_elements.push_back(hbox(std::move(sign_elements)));
			std::string signed_text = GAP + "signed " + (commit.signatures.size() > 1 ? std::to_string(commit.signatures.size()) + "x" : "");
...
auto renderGitStyle(cpplibostree::OSTreeRepo repo,
				std::vector<std::string> visible_commit_map,
				std::unordered_map<std::string, bool>  visible_branches,
				std::unordered_map<std::string, Color> branch_color_map,
				size_t selected_commit)
	...
	for (auto visible_commit_index : visible_commit_map) {
		...
		// check if it is first branch usage
		bool is_first_branch_usage = ! used_branches.at(relevant_branch);
		used_branches.at(relevant_branch) = true;
		...
		for (auto branch : used_branches) {
			if (branch.second) {
				if (branch.first == relevant_branch) {
					tree_top_elements.push_back(text(COMMIT_NODE_GIT));
				} else {
					tree_top_elements.push_back(text(COMMIT_TREE) | color(branch_color_map[branch.first]));
				}
				tree_root_elements.push_back(text(COMMIT_TREE) | color(branch_color_map[branch.first]));
		...
		Element commit_top_text_element = text(commit_top_text) | color(Color::Yellow);
		// branch HEAD
		if (is_first_branch_usage) {
			commit_top_text_element = hbox({
				commit_top_text_element,
				text("(") | color(Color::Yellow),
				text("HEAD") | color(Color::Cyan) | bold,
				text(" -> ") | color(Color::Yellow),
				text(relevant_branch) | color(branch_color_map.at(relevant_branch)) | bold,
				text(")") | color(Color::Yellow)
			});
		}
		// signed
		if (repo.isCommitSigned(commit)) {
			...
			commit_top_text_element = hbox({
				commit_top_text_element,
				text(signed_text) | color(Color::Green) | bold
			});
		}
		// selected
		if (marked_string.compare(commit.hash) == 0) {
		...
		comm_elements.push_back(text(GAP + commit.date));
		tree_elements.push_back(hbox(std::move(sign_elements)));
		comm_elements.push_back(text(GAP + INDENT + commit.subject));
		comm_elements.push_back(text(""));
  	}
	auto commitrender = hbox({
		vbox(std::move(tree_elements)),
...
auto commitRender(cpplibostree::OSTreeRepo repo,
	...
	switch (style) {
		case GIT:
			return renderGitStyle(repo, visible_commit_map, visible_branches, branch_color_map, selected_commit);
		default:
			return renderDefault(repo, visible_commit_map, visible_branches, branch_color_map, selected_commit);
	}
}
