    ...
    ostreeRepo(repo), selectedCommit(0), screen(ftxui::ScreenInteractive::Fullscreen())
{
	...
	// filter
	filterManager = std::unique_ptr<BranchBoxManager>(new BranchBoxManager(ostreeRepo, visibleBranches));
	filterView = Renderer(filterManager->branchBoxes, [&] {
...
void OSTreeTUI::refresh_commitComponents() {
	...
	for (auto& hash : visibleCommitViewMap) {
		commitComponents.push_back(
			CommitRender::CommitComponent(i, hash, *this)
		);
		i++;
	}
...
bool OSTreeTUI::promoteCommit(std::string hash, std::string branch, std::vector<std::string> metadataStrings, std::string newSubject, bool keepMetadata) {
	bool success = ostreeRepo.promoteCommit(hash, branch, metadataStrings, newSubject, keepMetadata);
